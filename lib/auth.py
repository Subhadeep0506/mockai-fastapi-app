...
def login_user(
    ...
    token_db = Token(
        user_id=fetched_user.id, access_token=access, refresh_token=refresh, status=True
    )
    try:
        db.add(token_db)
        db.commit()
...
def get_user_info(dependencies, db: Session):
    user_id = decodeJWT(dependencies)["id"]
    try:
...
def delete_user_by_id(user_id: str, dependencies, db: Session):
    requester_id = decodeJWT(dependencies)["id"]
    try:
